...
from PyGMO.util.coco.postproc import readalign
from PyGMO.util.coco.postproc.ppfig import saveFigure
from PyGMO.util.coco.postproc import toolsdivers
...
def figure_caption():
    if isinstance(targets, pproc.TargetValues):
...
def main(dsList0, dsList1, outputdir="", verbose=True):
    ...
    for f in funcs:
        ...
        filename = os.path.join(outputdir, 'ppscatter_f%s' % f)
