...
def annotate(entry0, entry1, dim, dims, minfvalue=1e-8, nbtests=1):
    ...
    if line[0][2] > 0 and line[1][2] > 0:
        ...
        annotcoord = [minfvalue, -line[1][1]/2 + 0.5 + offset*(len(dims)-dims.index(dim))]
        #if va == 'top':
        #    va = 'bottom'
    elif line[1][2] == 0:
        trans = blend(ax.transData, ax.transAxes)
        annotcoord = [minfvalue, line[0][1]/2 + 0.5 - offset*(len(dims)-dims.index(dim))]
...
def main(dsList0, dsList1, minfvalue=1e-8, outputdir='', verbose=True):
    ...
    for func in set.intersection(set(dictFun0), set(dictFun1)):
        ...
        filename = os.path.join(outputdir,'ppfig2_f%s' % str(func))
        ...
        for i, dim in enumerate(dims):
            ...
            kwargs = styles[i%(len(styles)/2)].copy()
        ...
        for i, dim in enumerate(dims):
            ...
            # annotation
            annotate(entry0, entry1, dim, dims, minfvalue, nbtests=nbtests)
