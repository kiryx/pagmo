...
from PyGMO.util.coco.postproc import toolsdivers, toolsstats, pproc, genericsettings
from PyGMO.util.coco.postproc.ppfig import saveFigure
from PyGMO.util.coco.postproc.pptex import color_to_latex, marker_to_latex, writeLabels
...
ecdfs_figure_caption_standard = (
                ...
                r"functions and subgroups in #1-D."
                )
...
def main(ds, sortedAlgs=None, target=ftarget_default, outputdir='./', verbose=True):
    ...
    if not os.path.isdir(outputdir):
        os.mkdir(outputdir)
    for f in dictFunc:
        filename = os.path.join(outputdir,'ppfigs_f%s' % str(f))
        handles = []
        ...
        for i, alg in enumerate(sortedAlgs):
            ...
            tmp = plt.plot([], [], label=str(alg).split(os.sep)[-1], **styles[i])
    ...
    try:
        ...
        for i in range(0, len(sortedAlgs)):
            symb = r'{%s%s}' % (color_to_latex(styles[i]['color']),
                                marker_to_latex(styles[i]['marker']))
            f.write((', ' if i > 0 else '') + '%s:%s' % (symb, writeLabels(str(sortedAlgs[i]))))
        f.close()
        if verbose:
