...
class DataSetList(list):
            ...
            if i == o:
                ...
                if set(i.instancenumbers).intersection(o.instancenumbers):
                    warnings.warn('instances ' + str(set(i.instancenumbers).intersection(o.instancenumbers))
                                  + (' found several times. Read data for F%s in %d-D' % (i.funcId, i.dim)) 
    ...
    def dictAll(self):
        ...
        sorted = {}
        for i in self:
                sorted.setdefault('All', DataSetList()).append(i)
        return sorted

    def getFunctions(self):
        """Returns a list of all functions read from log files.
        """
        funcs = []
        for i in self:
            funcs.append(str(i.funcId))
        
        return funcs
    
    def setFunctionGroups(self, groups):
        """Set function groups which will be used in this module.
        """
        self.function_groups = {}
        if isinstance(groups, dict):
            funcs = []
            for s in self: #get all functions
                funcs.append(s.funcId)

            for key in groups.keys():
                if isinstance(groups[key], list):
                    for i in groups[key]:
                        if i not in funcs: #check if function exists in data
                            raise ValueError("Function "+str(i)+" not found.")
                        
                        self.function_groups.setdefault(key, i)
                else:
                    raise ValueError("Given dictionary should be like {key1:[\'Schwefel\',...]}")
        self.function_groups = groups

    def GetFunctionGroups(self):
        """Get function groups which are set.
        """
    ...
    def dictByFuncGroup(self):
        ...
        The output dictionary has function group number {0..n} as keys and the
        corresponding slices as values.

        """
        grps = {}
        if isinstance(self.function_groups, dict):
            for key in self.function_groups.keys():
                for fun in self.function_groups[key]:
                    for o in self:
                        if fun == o.funcId:
                            grps.setdefault(key, DataSetList()).append(o)
    ...
    def info(self, opt=None):
        ...
        if len(self) > 0:
            ...
            print '%d Function%s with ID%s %s' % (nbfuns, splural, splural, ', '.join(functions))
    ...
    def det_best_data_lines(self, target_values, fct, dim,
        ...
        if any(line is None for line in best_lines):
            warnings.warn('best data lines for %s in %s-D could not be determined'
                          % (str(fct), str(dim)))
    ...
    for f in funcs:
        for alg in dictAlg:
            ...
            try:
                ...
                txt = ('No data for algorithm %s on function %s.'
                       % (alg, f)) # This message is misleading.
                warnings.warn(txt)

            if res.setdefault(f, {}).has_key(alg):
                txt = ('Duplicate data for algorithm %s on function %s-D.'
                       % (alg, f))
                warnings.warn(txt)
...
def dictAlgAll(dictAlg):
    """Returns a dictionary with `All` group as key.

    This method is meant to be used with an input argument which is a
    dictionary with algorithm names as keys and which has list of
    :py:class:`DataSet` instances as values.
    The resulting dictionary will have a string denoting `All` group
